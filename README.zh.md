...
如果你更想彻底自己掌管 MCP 配置,而不是一直读另外四个工具的文件,那就导入一次然后停止借用:

```sh
dsh-mcp-json-import --dry-run   # 先看会写入什么
dsh-mcp-json-import             # 写入 ~/.dsh/mcp.json
```

然后在 `~/.dsh/settings.yaml` 的 `mcp-json` 段里设 `borrow: false`。发现范围收窄到 `~/.dsh/mcp.json` 与 `<cwd>/.dsh/mcp.json`,文件归你,不会有任何东西改写它。

| 参数 | 含义 |
|---|---|
| `--target <路径>` | 写入的目标文档,默认 `~/.dsh/mcp.json`。 |
| `--cwd <路径>` | 项目级各层解析时的基准目录。 |
| `--force` | 覆盖目标里已存在的同名条目。不加则保留你自己的。 |
| `--dry-run` | 只报告会有什么变化,不写文件。 |

条目在写入时会被翻译成 Claude 词汇,所以 OpenCode 或 Codex 的 server 进来后是 `command` 加 `args`。目标已定义的名字会被保留而非替换,因为你亲手写的那条才是关于这台 server 的更具体的陈述。目标里其他顶层键会保留:只有 `mcpServers` 段归本命令管。

这是一条你主动执行的命令,不是插件自动做的事,而这正是整个设计的要点。一份每次启动都从其他工具重写的文档,不可能同时又是你手编的文档——你加的条目会被覆盖,你删掉的借来 server 会在下次调和时复活。导入一次,之后文件就归你了。

另外注意,导入是复制条目,并不改变哪一层胜出。项目的 `.mcp.json` 依然压过 `~/.dsh/mcp.json`,所以关掉 `borrow` 是这条指引的后半段,不是可选的收尾。
