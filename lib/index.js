...
function normalizeDocument(document, dialect) {
	...
	if (dialect === "opencode") {
		const section = document.mcp;
		if (section === void 0) return void 0;
		if (!isRecord(section)) throw new Error("has an \"mcp\" section that is not an object");
		return Object.fromEntries(Object.entries(section).filter((pair) => isRecord(pair[1])).map(([name, entry]) => [name, fromOpenCode(entry)]));
	}
...
function toClientConfig(server, defaultCwd) {
	...
	if (!SERVER_NAME_PATTERN.test(name)) throw new McpJsonEntryError(server, `has a name the harness cannot use as a tool namespace; names must match ${String(SERVER_NAME_PATTERN)}`);
...
function apply(ctx, config) {
	const userPath = expandHome(config.userPath ?? "~/.dsh/mcp.json");
...
//#endregion
export { Config, DEFAULT_USER_PATH, McpJsonEntryError, apply, discover, expandHome, inject, isDisabled, layerSources, name, normalizeDocument, parseJsonc, readLayer, stripJsonExtras, toClientConfig };
